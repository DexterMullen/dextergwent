...
class Game {
	...
	// redraw up to 3 crds at the beggining of the game
	async initialRedraw() {
		if (player_op.controller instanceof ControllerAI) {
			for (let i = 0; i < 3; i++) player_op.controller.redraw();
		}
		if (player_me.controller instanceof ControllerAI) {
			for (let i = 0; i < 3; i++) player_me.controller.redraw();
		} else {
			await ui.queueCarousel(player_me.hand, 3, async (c, i) => await player_me.deck.swap(c, c.removeCard(i)), c => true, true, true, "Choose up to 3 cards to redraw");
